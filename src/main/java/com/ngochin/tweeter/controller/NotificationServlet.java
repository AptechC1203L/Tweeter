...
public class NotificationServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (pi.length() > 1) {
            ...
            NotificationDao dao = f.getNotificationDao();
            Notification n = dao.getNotification(id);
            
            n.setIsRead(true);
            dao.saveNotification(n);

            response.sendRedirect(getServletContext().getContextPath() + n.getLink());
        } else {
            RequestDispatcher rd = request.getRequestDispatcher("/all-notifications.jsp");
